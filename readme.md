...
* 7 월 30일(추가)
  * 로그인까지는 구현 완료
  * Java Sellenium의 Key를 활용하여 Command와 V누르는 것을 구현
  * keyDown이 아닌 sendKey를 활용
  * 로그인 후에는 확인하지 않은 이메일 갯수 확인 => 0이 아닐 경우 메일함으로 이동
  * 고쳐야 할 부분
    * 현재 확인하지 않은 이메일 갯수를 확인하지 못하고있다
    * findElement(By.className)이 제대로 작동을 안함
    * 왜지?
...
* 7월 31일
  ...
  * 이메일 목록 페이지 이동 완료
  * 읽지 않은 메일 갯수 확인 완료
  * 현재 화면에서 메일목록 중 읽지않는 메일(상단의 n개) 뽑을수 있도록 구현해야한다
  * ClassName사용하면 될거같은데..
  * 구현해야 할 것
    * 확인하지 않은 메일들 선택하기
    * 카카오 메세지 보내기 기능 구현
  * 구현하고 싶은 것
    * 자바 스케쥴링이나 타이머 적용해보기?
    * 처음 api요청을 보내고 일정주기(30분, 1시간등등...)로 이메일 확인해주는 기능
